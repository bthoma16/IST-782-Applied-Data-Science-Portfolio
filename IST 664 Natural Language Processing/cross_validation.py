'''
    This file contains functions for cross_validation.
    Usage:  Given the previously defined featuresets and list of labels   
        num_folds = 10  # or 5
        label_list = ['pos','neg']
...
# Using NLTK to calculate evaluation measures from a reflist of gold labels and a testlist of predicted labels for all labels in a list
# It returns lists of precision and recall for each label
def eval_measures(reflist, testlist, label_list):
    ...
    #   Storing them in sets for each label, stored in dictionaries
    ...
    precision_list = []
    recall_list = []
    # Calculating precision and recall for all labels using the NLTK functions
    for lab in label_list:
...
# This function calculates F-measure (beta = 1) from precision and recall
...
# This function calculates the cross-validation and creates classifier models for each fold
#    In each fold, it applies the model to the reference set, obtaining a list of predicted labels
#    The final list collects all the reference/gold labels and test/predicted labels
def naive_bayes(num_folds, featuresets, label_list):
    ...
    for i in range(num_folds):
        ...
        # Evaluating against test_this_round and save accuracy
        accuracy_this_round = nltk.classify.accuracy(classifier, test_this_round)
        print(i, accuracy_this_round)
        accuracy_list.append(accuracy_this_round)
		
        # Adding the gold labels and predicted labels for this round to the overall lists
        for (features, label) in test_this_round:
            reflist.append(label)
            testlist.append(classifier.classify(features))
    print('Done with cross-validation')
    # Calling the evaluation measures function    
